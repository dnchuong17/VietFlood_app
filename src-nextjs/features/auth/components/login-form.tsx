...
export function LoginForm() {
    ...
    async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        logger.debug('LoginForm', 'Submit started', { username });

        if (!username.trim() || !password.trim()) {
            logger.warn('LoginForm', 'Submit failed - missing credentials');
            showAlert({
        ...
        try {
            ...
            logger.info('LoginForm', 'Attempting sign-in', { username });
            ...
            logger.info('LoginForm', 'Sign-in successful', { username, userRole });
            ...
            if (userRole === "relief") {
                logger.debug('LoginForm', 'Redirecting to relief dashboard');
                router.replace("/quan-ly");
            } else {
                logger.debug('LoginForm', 'Redirecting to citizen dashboard');
                router.replace("/trang-chu");
            }
            router.refresh();
        } catch (error) {
            logger.error('LoginForm', 'Sign-in failed', error instanceof Error ? error : new Error(String(error)), { username });
